...
แถวเกิดตอนเปิด session (ก่อนสแกน) ไม่ใช่ตอนยื่นคำร้อง
`applicant_id` เติมทีหลังตอนยื่นคำร้องสำเร็จ

นโยบายเก็บข้อมูล: เก็บถาวรเฉพาะแถวที่ผูก applicant แล้ว — แถวกำพร้า (`applicant_id IS NULL`)
มีอายุแค่ถึงรอบสแกนถัดไปหรือการยื่นคำร้องถัดไปของ person เดิม แล้วถูกกวาดทิ้ง
(ดู `services.liveness_link.sweep_orphan_attempts`) สถิติ fail/skip ที่เหลือใน DB
จึงเป็น "รอบที่ถูกแนบกับคำร้อง" เท่านั้น — ภาพรวม outage ของ AINU ต้องดูจาก server log
