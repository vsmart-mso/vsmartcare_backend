...
class DashboardCaseRow(BaseModel):
    """แถวคำร้อง — ฟิลด์สอดคล้อง `CaseForStaffRead` เพื่อให้ตารางหน้าบ้านใช้ซ้ำได้."""
...
class DashboardCasesRead(BaseModel):
    """รายการคำร้องสำหรับหน้า dashboard — ไม่ส่ง province_id = ทั้งประเทศ."""

    province_ids: list[int] | None = Field(
        None,
        description="จังหวัดที่กรอง; null = ทุกจังหวัด",
    )
    page: int
    page_size: int
    total_items: int = Field(..., description="จำนวนคำร้องหลังใช้ filter")
    total_pages: int
